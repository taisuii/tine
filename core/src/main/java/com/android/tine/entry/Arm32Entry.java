package com.android.tine.entry;
...
import com.android.tine.utils.ThreeTuple;
...
public final class Arm32Entry {
    ...
    private static final boolean USE_HARDFP = TineConfig.sdkLevel >= Build.VERSION_CODES.M;
    ...
    private static final boolean DISALLOW_LONG_CROSS_CR_AND_STACK = TineConfig.sdkLevel >= Build.VERSION_CODES.S;
    ...
    private static Object handleBridge(int artMethod, int originExtras, int sp) throws Throwable {
        ...
        int extras = (int) Tine.cloneExtras(originExtras);
        Tine.log("handleBridge: artMethod=%#x originExtras=%#x extras=%#x sp=%#x", artMethod, originExtras, extras, sp);
        Tine.HookRecord hookRecord = Tine.getHookRecord(artMethod);
        ...
        long thread = Tine.currentArtThread0();
        ...
        if (hookRecord.isStatic) {
            ...
            receiver = Tine.getObject(thread, coreRegisters[0]);
            crIndex = 1;
            stackIndex = 1;
        }
        ...
        if (hookRecord.paramNumber > 0) {
            ...
            for (int i = 0;i < hookRecord.paramNumber;i++) {
                ...
                if (paramType == double.class) {
                    ...
                    if (paramType.isPrimitive()) {
                        ...
                        value = Tine.getObject(thread, asInt);
                    }
                }
                args[i] = value;
                stackIndex++;
            ...
            args = Tine.EMPTY_OBJECT_ARRAY;
        }

        return Tine.handleCall(hookRecord, receiver, args);
    }

    private static ThreeTuple<int[], int[], float[]> getArgs(Tine.HookRecord hookRecord, int extras, int sp) {
        int crLength;
        int stackLength;
        int fpLength;
        ...
        Tine.getArgsArm32(extras, sp, coreRegisters, stack, fpRegisters);
