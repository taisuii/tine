...
public final class X86Entry {
    ...
    private static Object handleBridge(int artMethod, int extras, int ebx) throws Throwable {
        Tine.log("handleBridge: artMethod=%#x extras=%#x ebx=%#x", artMethod, extras, ebx);
        ...
        if (hookRecord.isStatic) {
            ...
            receiver = Tine.getObject(thread, argsAsInts[0]);
            index = 1;
        }
        ...
        if (hookRecord.paramNumber > 0) {
            ...
            for (int i = 0;i < hookRecord.paramNumber;i++) {
                ...
                if (paramType.isPrimitive()) {
                    ...
                    value = Tine.getObject(thread, argsAsInts[index]);
                }
                args[i] = value;
                index++;
            }
    ...
    private static int[] getArgsAsInts(Tine.HookRecord hookRecord, int extras, int ebx) {
        ...
        Tine.getArgsX86(extras, array, ebx);
        return array;
    }
}
