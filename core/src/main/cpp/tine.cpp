...
bool TineConfig::debug = false;
bool TineConfig::debuggable = false;
bool TineConfig::anti_checks = false;
bool TineConfig::jit_compilation_allowed = true;
bool TineConfig::auto_compile_bridge = false;

EXPORT_C void TineSetAndroidVersion(int version) {
    Android::version = version;
}

EXPORT_C void* TineOpenElf(const char* elf) {
    return new ElfImage(elf);
}

EXPORT_C void TineCloseElf(void* handle) {
    delete static_cast<ElfImage*>(handle);
}

EXPORT_C void* TineGetElfSymbolAddress(void* handle, const char* symbol, bool warn_if_missing) {
...
void* TineSuspendVM(JNIEnv* env) {
    return new ScopedSuspendVM(art::Thread::Current(env));
}

void TineResumeVM(void* handle) {
...
EXPORT_C bool TineNativeInlineHookSymbolNoBackup(const char* elf, const char* symbol, void* replace) {
    ElfImage handle(elf);
...
EXPORT_C void TineNativeInlineHookFuncNoBackup(void* target, void* replace) {
    TrampolineInstaller::GetOrInitDefault()->NativeHookNoBackup(target, replace);
}

EXPORT_C void TineFillWithNop(void* target, size_t size) {
    TrampolineInstaller::GetOrInitDefault()->FillWithNop(target, size);
}

void Tine_init0(JNIEnv* env, jclass Tine, jint androidVersion, jboolean debug, jboolean debuggable,
        jboolean antiChecks, jboolean disableHiddenApiPolicy, jboolean disableHiddenApiPolicyForPlatformDomain) {
    if (debug == JNI_TRUE) LOGI("Tine native init...");
    TineConfig::debug = static_cast<bool>(debug);
    TineConfig::debuggable = static_cast<bool>(debuggable);
    TineConfig::anti_checks = static_cast<bool>(antiChecks);
    ...
    Android::Init(env, androidVersion, disableHiddenApiPolicy, disableHiddenApiPolicyForPlatformDomain);
    {
        ...
        ScopedLocalClassRef I(env, "com/android/tine/Ruler$I");
...
if (auto field = env->GetStaticFieldID(Tine, (name), (sig)); (sig)[0] == 'I') env->SetStaticIntField(Tine, field, (value)); \
else env->SetStaticLongField(Tine, field, (value));

    SET_JAVA_VALUE("arch", "I", kCurrentArch);
    SET_JAVA_VALUE("openElf", "J", reinterpret_cast<jlong>(TineOpenElf));
    SET_JAVA_VALUE("findElfSymbol", "J", reinterpret_cast<jlong>(TineGetElfSymbolAddress));
    SET_JAVA_VALUE("closeElf", "J", reinterpret_cast<jlong>(TineCloseElf));
    ...
    SET_JAVA_VALUE("suspendVM", "J", reinterpret_cast<jlong>(TineSuspendVM));
    SET_JAVA_VALUE("resumeVM", "J", reinterpret_cast<jlong>(TineResumeVM));
...
jobject Tine_hook0(JNIEnv* env, jclass, jlong threadAddress, jclass declaring, jobject hookRecord,
    ...
    if (TineConfig::jit_compilation_allowed && TineConfig::auto_compile_bridge) {
    ...
    bool skip_first_few_bytes = TineConfig::anti_checks
...
jobject Tine_hookReplace(JNIEnv* env, jclass, jlong threadAddress, jclass declaring, jobject hookRecord,
    ...
    bool skip_first_few_bytes = TineConfig::anti_checks && is_inline_hook
...
jlong Tine_getArtMethod(JNIEnv* env, jclass, jobject javaMethod) {
    return static_cast<jlong>(reinterpret_cast<intptr_t>(
            art::ArtMethod::FromReflectedMethod(env, javaMethod)));
}

jboolean Tine_compile0(JNIEnv* env, jclass, jlong thread, jobject javaMethod) {
    return static_cast<jboolean>(art::ArtMethod::FromReflectedMethod(env, javaMethod)->Compile(
            reinterpret_cast<art::Thread*>(thread)));
}

jboolean Tine_decompile0(JNIEnv* env, jclass, jobject javaMethod, jboolean disableJit) {
    return static_cast<jboolean>(art::ArtMethod::FromReflectedMethod(env, javaMethod)->Decompile(
            disableJit));
}

jboolean Tine_disableJitInline0(JNIEnv*, jclass) {
    return static_cast<jboolean>(art::Jit::DisableInline());
}

void Tine_setJitCompilationAllowed(JNIEnv*, jclass, jboolean allowed, jboolean autoCompileBridge) {
    TineConfig::jit_compilation_allowed = JBOOL_TRUE(allowed);
    TineConfig::auto_compile_bridge = JBOOL_TRUE(autoCompileBridge);
}

jboolean Tine_disableProfileSaver0(JNIEnv*, jclass) {
    return static_cast<jboolean>(Android::DisableProfileSaver());
}

jobject Tine_getObject0(JNIEnv* env, jclass, jlong thread, jlong address) {
    return reinterpret_cast<art::Thread*>(thread)->AddLocalRef(env, reinterpret_cast<Object*>(address));
}

jlong Tine_getAddress0(JNIEnv*, jclass, jlong thread, jobject o) {
    return reinterpret_cast<jlong>(reinterpret_cast<art::Thread*>(thread)->DecodeJObject(o));
}

#ifdef __aarch64__
void Tine_getArgsArm64(JNIEnv* env, jclass, jlong javaExtras, jlong sp, jbooleanArray typeWides,
...
#elif defined(__arm__)
void Tine_getArgsArm32(JNIEnv *env, jclass, jint javaExtras, jint sp,
...
#elif defined(__i386__)
void Tine_getArgsX86(JNIEnv* env, jclass, jint javaExtras, jintArray javaArray, jint ebx) {
    auto extras = reinterpret_cast<Extras*>(javaExtras);
...
void Tine_syncMethodInfo(JNIEnv* env, jclass, jobject javaOrigin, jobject javaBackup, jboolean skipDeclaringClass) {
...
void Tine_setDebuggable(JNIEnv*, jclass, jboolean debuggable) {
    TineConfig::debuggable = static_cast<bool>(debuggable);
}

void Tine_disableHiddenApiPolicy0(JNIEnv*, jclass, jboolean application, jboolean platform) {
    Android::DisableHiddenApiPolicy(application, platform);
}

jlong Tine_currentArtThread0(JNIEnv* env, jclass) {
...
jlong Tine_cloneExtras(JNIEnv*, jclass, jlong extras) {
...
void Tine_enableFastNative(JNIEnv* env, jclass Tine) {
    LOGI("Experimental feature FastNative is enabled.");
    for (auto& method_info : gFastNativeMethods) {
        auto method = art::ArtMethod::Require(env, Tine, method_info.name, method_info.signature, true);
...
static const JNINativeMethod gMethods[] = {
        {"init0", "(IZZZZZ)V", (void*) Tine_init0},
        {"enableFastNative", "()V", (void*) Tine_enableFastNative},
        {"getArtMethod", "(Ljava/lang/reflect/Member;)J", (void*) Tine_getArtMethod},
        {"hook0", "(JLjava/lang/Class;Lcom/android/tine/Tine$HookRecord;Ljava/lang/reflect/Member;Ljava/lang/reflect/Method;ZZZ)Ljava/lang/reflect/Method;", (void*) Tine_hook0},
        {"hookReplace0", "(JLjava/lang/Class;Lcom/android/tine/Tine$HookRecord;Ljava/lang/reflect/Member;Ljava/lang/reflect/Method;Ljava/lang/reflect/Method;ZZZ)Ljava/lang/reflect/Method;", (void*) Tine_hookReplace},
        {"compile0", "(JLjava/lang/reflect/Member;)Z", (void*) Tine_compile0},
        {"decompile0", "(Ljava/lang/reflect/Member;Z)Z", (void*) Tine_decompile0},
        {"disableJitInline0", "()Z", (void*) Tine_disableJitInline0},
        {"setJitCompilationAllowed0", "(ZZ)V", (void*) Tine_setJitCompilationAllowed},
        {"disableProfileSaver0", "()Z", (void*) Tine_disableProfileSaver0},
        {"syncMethodInfo", "(Ljava/lang/reflect/Member;Ljava/lang/reflect/Method;Z)V", (void*) Tine_syncMethodInfo},
        {"getObject0", "(JJ)Ljava/lang/Object;", (void*) Tine_getObject0},
        {"getAddress0", "(JLjava/lang/Object;)J", (void*) Tine_getAddress0},
...
#ifdef __aarch64__
        {"getArgsArm64", "(JJ[Z[J[J[D)V", (void*) Tine_getArgsArm64}
#elif defined(__arm__)
        {"getArgsArm32", "(II[I[I[F)V", (void*) Tine_getArgsArm32}
#elif defined(__i386__)
        {"getArgsX86", "(I[II)V", (void*) Tine_getArgsX86}
#endif
};

bool register_Tine(JNIEnv* env, jclass Tine) {
    return LIKELY(env->RegisterNatives(Tine, gMethods, NELEM(gMethods)) == JNI_OK);
}
